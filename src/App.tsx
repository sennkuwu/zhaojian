...
export default function App() {
  ...
  const [hasSavedLamp, setHasSavedLamp] = useState(false)
  ...
  useEffect(() => {
    ...
    const savedLamps = loadStore().lamps
    if (savedLamps.length) { setLamp(savedLamps[savedLamps.length - 1]); setHasSavedLamp(true) }
  }, [])
  ...
  function reset() { clearStore(); setLamp(null); setHasSavedLamp(false); setLabel(''); setMessage(''); setView('home'); setNotice('当前设备的数据已清除，且不可恢复。') }
  ...
  return <main className="app-shell">
    ...
    {view === 'home' && <section className="hero"><p className="eyebrow">中元夜 · 私密纪念作品</p><h1>为想念的人<br /><em>留一盏灯</em></h1><p className="lede">有些人离开很久了。<br />却仍在我们生活的细节里。</p><button className="primary" onClick={begin}>留一盏灯 <span>→</span></button><button className="secondary" onClick={() => setInfo(true)}>先在这里看看</button>{hasSavedLamp && <button className="secondary saved-entry" onClick={() => { setNotice('已经亮了。'); setView('lit') }}>查看这台设备上的灯</button>}</section>}
    {view === 'compose' && <section className="panel"><p className="eyebrow">写下，或不写</p><h2>让这一刻有一个<br />只有你知道的称呼。</h2><label>此刻，你想起了谁？<input maxLength={24} value={label} onChange={event => { setLabel(event.target.value); saveDraft(event.target.value, message) }} placeholder="一个只有你知道的称呼" aria-describedby="label-hint" /><span id="label-hint" className="counter">{labelCount}/24</span></label><label>如果只能留一句话，你想对 Ta 说什么？<textarea maxLength={120} rows={3} value={message} onChange={event => { setMessage(event.target.value); saveDraft(label, event.target.value) }} placeholder="最近的日子，我过得还不错。" aria-describedby="message-hint" /><span id="message-hint" className="counter">{messageCount}/120</span></label><p className="privacy">这句话只保存在当前设备。称呼和留言都可以留空。</p><button className="primary" onClick={() => setView('hold')}>下一步 <span>→</span></button><button className="secondary" onClick={() => setView('home')}>返回</button></section>}
    {scene && <section className="scene" style={style}><div className="stars" aria-hidden="true" /><div className={`lamp ${view === 'release' ? 'releasing' : view === 'lit' ? 'lit' : ''}`} onAnimationEnd={handleAnimationEnd} aria-hidden="true" /><div className="scene-copy" aria-live="polite"><p className="eyebrow">{view === 'hold' ? '点灯' : view === 'release' ? '升灯' : '你的灯'}</p><h2>{notice}</h2>{view === 'hold' && <p>请轻轻按住它，让这句话在今晚亮起来。</p>}{view === 'release' && <p>让它慢慢升入夜色。你也可以现在留在灯下。</p>}{view === 'lit' && <p>有些话不必抵达。<br />被想起，便有了归处。</p>}</div>{view === 'hold' && <p id="hold-status" className="sr-only" aria-live="polite">{progress > 0 ? `正在点亮，还需约 ${Math.max(1, Math.ceil((1 - progress) * 3))} 秒` : '持续按住三秒点灯'}</p>}{view === 'hold' && <button className="hold-button" aria-describedby="hold-status" onPointerDown={startHold} onPointerUp={stopHold} onPointerCancel={stopHold} onKeyDown={keyDown} onKeyUp={keyUp} aria-label="持续按住三秒点灯">按住这里<br /><strong>{Math.round(progress * 100)}%</strong></button>}{view === 'release' && <button ref={releaseSkip} className="release-skip secondary" onClick={completeAscent}>跳过升灯</button>}{view === 'lit' && <div className="actions"><button className="primary" onClick={makeCard}>带走这束光 <span>→</span></button><button className="secondary" onClick={begin}>再留一盏</button></div>}</section>}
    ...
    {info && <div className="modal-backdrop" role="presentation" onClick={() => setInfo(false)}><aside className="modal" role="dialog" aria-modal="true" aria-labelledby="about-title" onClick={event => event.stopPropagation()}><button className="close" onClick={() => setInfo(false)}>×</button><p className="eyebrow">关于《照见》</p><h2 id="about-title">一盏灯，也足以照见一个人。</h2><p>这是一次短暂、私密的网络纪念仪式。私密灯只保存在当前设备，环境光不代表其他用户。这里不提供通灵、祈福结果或任何人的回应。</p><p>我们不要求真实姓名，也不收集身份、位置或设备指纹。清除浏览器数据后，私密内容无法恢复。</p></aside></div>}
  </main>
}
